...
with_pct_change as (
    select
        ...
        rate_change_1m,
        rate_change_3m,
        -- % change in rolling volume vs prior 3-month window
        safe_divide(
            rolling_3m_avg_txns - lag(rolling_3m_avg_txns, 3) over (
                partition by region, property_type order by month
            ),
...
sensitivity as (
    select
        ...
        -- A large negative value = volumes fall when rates rise
    ...
    group by 1, 2
    having months_observed >= 12  -- require at least a year of data
)
...
select
    ...
    -- Composite sensitivity: magnitude of response to rate moves
    round(
